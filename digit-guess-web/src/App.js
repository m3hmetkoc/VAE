...
function App() {
  ...
  const predictDigit = async (isAutomatic = false) => {
    ...
    try {
      ...
      // Debug: Log canvas data to console (only for manual predictions)
      if (!isAutomatic) {
        console.log('Canvas data (28x28 = 784 pixels):', canvasData);
        console.log('Array length:', canvasData.length);
        console.log('Data type:', typeof canvasData[0]);
        console.log('Min value:', Math.min(...canvasData));
        console.log('Max value:', Math.max(...canvasData));
        
        // Also log as 2D matrix for easier visualization
        const matrix = [];
        for (let i = 0; i < 28; i++) {
          matrix.push(canvasData.slice(i * 28, (i + 1) * 28));
        }
        console.log('As 2D matrix (28x28):', matrix);
      }
            ...
            {error && (
              <div className="error-message">
                ...
                <p className="error-hint">
                  Make sure the Flask server is running: <code>python prediction_server.py</code>
                </p>
              </div>
            )}
